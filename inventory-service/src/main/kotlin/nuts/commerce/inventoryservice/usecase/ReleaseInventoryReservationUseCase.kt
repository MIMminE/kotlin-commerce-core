...
@Component
class ReleaseInventoryReservationUseCase(
    ...
    fun execute(inventoryId: UUID, qty: Long): Result {
        ...
        val payloadObj = InventoryUpdatedPayload(saved.inventoryId, saved.productId, saved.quantity)
        val payload = objectMapper.writeValueAsString(payloadObj)
        val outbox = OutboxRecord.create(inventoryId = saved.inventoryId, eventType = OutboxEventType.INVENTORY_UPDATED, payload = payload)
    ...
    private data class InventoryUpdatedPayload(val inventoryId: UUID, val productId: UUID, val quantity: Long)
}
