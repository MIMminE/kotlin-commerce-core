...
- 주문, 재고, 결제, 상품 서비스를 이벤트로 연결한 커머스 MSA 흐름 설계
- Transactional Outbox로 DB 저장과 메시지 발행 사이의 신뢰성 문제 해결
- Lease 기반 publisher 경합 제어와 eventId 기반 consumer 멱등 처리
- Idempotency-Key를 활용한 주문 생성 API 중복 요청 방어
- Docker Compose 기반 로컬 재현 환경과 웹 클라이언트를 통한 주문 흐름 검증
...
| 문제 | 설계 대응 |
| --- | --- |
| 주문 저장은 성공했지만 Kafka 발행이 실패할 수 있음 | 주문 저장 트랜잭션 안에서 Outbox 레코드를 함께 저장하고, 별도 publisher가 재시도 |
| publisher가 여러 인스턴스로 뜨면 같은 이벤트를 동시에 발행할 수 있음 | `leaseOwner`, `leaseUntil` 기반 claim-and-lock 방식으로 발행 대상 선점 |
| Kafka consumer는 같은 이벤트를 두 번 받을 수 있음 | `eventId` 처리 이력으로 consumer 멱등성 확보 |
| 클라이언트가 주문 생성 요청을 재전송할 수 있음 | `Idempotency-Key`로 중복 주문 생성 방어 |
| 상품 조회와 재고 변경의 요구사항이 다름 | 조회는 Redis 캐시, 변경은 이벤트 기반 갱신/무효화 흐름으로 분리 |
...
| 설계 선택 | 이유 |
| --- | --- |
| 서비스별 DB 소유 | 다른 서비스 테이블을 직접 수정하지 않고 이벤트로 상태를 전달하는 MSA 경계를 보여주기 위해 |
| Transactional Outbox | DB 트랜잭션과 Kafka 발행을 직접 묶을 수 없는 문제를 현실적인 방식으로 다루기 위해 |
| Lease 기반 publisher | 단일 publisher 전제에 기대지 않고 다중 인스턴스 실행을 고려하기 위해 |
| Consumer 멱등 처리 | exactly-once 전달을 기대하기보다 중복 수신에도 결과가 망가지지 않는 구조를 만들기 위해 |
| 웹 클라이언트 추가 | 백엔드 패턴을 API 목록으로만 설명하지 않고 주문 흐름을 눈으로 따라갈 수 있게 하기 위해 |

이 프로젝트는 “Kotlin/Spring Boot로 MSA를 만들 수 있다”보다 “분산 환경에서 발생하는 신뢰성 문제를 어떤 패턴으로 완화할 수 있는지 설명할 수 있다”는 점을 보여주기 위한 포트폴리오입니다.
