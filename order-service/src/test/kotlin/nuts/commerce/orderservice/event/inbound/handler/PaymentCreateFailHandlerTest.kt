...
class PaymentCreateFailHandlerTest {
    ...
    private lateinit var handler: PaymentCreateFailHandler
    ...
    fun setUp() {
        ...
        handler = PaymentCreateFailHandler(
            orderRepository,
            sageRepository,
            outboxRepository,
            objectMapper
    ...
    @Test
    fun `결제 생성 실패 - 주문 상태가 PAYMENT_FAILED로 변경되고 예약 해제 요청 outbox가 생성된다`() {
        // given
        ...
        val event = OrderInboundEvent(
            ...
            payload = PaymentCreationFailedPayload(
        ...
        assertEquals(OutboundEventType.PAYMENT_RELEASE_REQUEST, outboxRecord.eventType)
    }
}
